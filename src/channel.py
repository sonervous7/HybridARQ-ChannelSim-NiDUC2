...
class Channel:
    def __init__(self, channel_type="BSC", **kwargs):
        ...
        if channel_type == "BSC":
            self.channel = komm.BinarySymmetricChannel(
                crossover_probability=kwargs.get("ber", 0.1)
            )
    ...
    def channel_transmit(self, data, as_bits=False):
        ...
        if as_bits:
            ...
            if self.channel_type == "BSC":
                return list(
                    self.channel(np.array(data))
                )  # komm obsługuje tablicę numpy
            else:
                return self.channel.transmit_bits(data)
        else:
            # Jeśli dane są w postaci bajtowej
            if self.channel_type == "BSC":
                bit_data = _bytes_to_bits(data)
                transmitted_bits = self.channel(bit_data)
                return _bits_to_bytes(transmitted_bits)
            else:
