...
class ErrorCorrectionCode:
    ...
    def encode(self, data_bytes):
        logger.info("Dane przed kodowaniem (RS):", data_bytes)
        encoded_data = self.rs.encode(data_bytes)
        # logger.error("Dane po kodowaniu (RS):", encoded_data)
    ...
    def decode(self, data_bytes):
        try:
            ...
            logger.info("Dane przed dekodowaniem (RS):", data_bytes)
            decoded_data, _, _ = self.rs.decode(
                data_bytes
            )  # (decoded_data, repair, erasures)
            # logger.error("Dane po dekodowaniu (RS):", decoded_data)
            return decoded_data
        except Exception as e:
            logger.error("Błąd korekcji Reed-Solomon:", e)
            return None
    ...
    def decode_bits(self, bit_array, original_bit_length):
        ...
        if decoded_bytes:
            ...
            # Debug logger.error
            logger.info("Długosc decoded_bytes: ", len(decoded_bits))
            logger.info(
                "Długosc parametru original_bit_length: ", len(original_bit_length)
            )
            logger.info(
                "Obliczona różnica aby mieć odpowiedni zakres: ", decoded_bits_diff
            )
